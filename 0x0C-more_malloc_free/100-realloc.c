 ...
 *
 * Return: pointer to the new memory block, or NULL if new_size
 * is 0 and ptr is not NULL,
...
void *_realloc(void *ptr, unsigned int old_size, unsigned int new_size)
{
	...
	/* If new_size is 0, free the memory block pointed to*/
	if (new_size == 0 && ptr != NULL)
	{
		free(ptr);
		return (NULL);
	}
	/* If ptr is NULL, allocate a new memory block of size*/
	if (ptr == NULL)
	{
